#   JULY 2019 (Q2-'19) Monthly Subs Report Filtering Methodology by Brand/File   #
...
#File path updated_BlueHost##
bh_raw <- read_csv(paste0(raw_msr_subs_directory, 'Bluehost_Monthly-Subs-Report_', filename_date, '.csv'))
...
ehost.active<-ehost[(ehost$`Term Expiration Date` >= as.Date(exp_date)),]
...
##File path updated vDeck##
vdeck_raw <- read_csv(paste0(raw_msr_subs_directory, 'vDeck_Monthly-Subs-Report_', filename_date, '.csv'))
...
# h<-h[(h$`Activation Date` <= as.Date(act_date))
#      & (h$exp >= as.Date(exp_date))
#      & ((h$`Cancellation Date` >= as.Date(exp_date)) | is.na(h$`Cancellation Date`)) 
#      & (h$`Activation Date` >= as.Date('1975-01-01'))
#      & (h$`Subscription Status` %in% c('Active')),]
...
setwd('P:/Stu_Nabila/Subscriber Balances/Adi/2019/2019Q3 - Subs Data - Jul/filtered')

write_csv(final, paste0('hg_hosting_msr_', report_date, '_filtered-test.csv'))

# HG LWP #

#hglwp_raw <- read_csv(paste0(raw_msr_subs_directory, 'hg_lwp_Monthly-Subs-Report_', filename_date, '.csv')) 
...
write_csv(ctct_final, 'ctct_msr_2019-07-31_filtered-test.csv')
...
a4<-a3[(a3$`Activation Date` <= as.Date(act_date)) 
...
s5_raw <- read_csv(paste0(raw_msr_subs_directory, 'S5_Monthly-Subs-Report_', filename_date, '.csv'), 
...
s5_filtered_p1_LAD_InRange_func<-function(s5_msr_raw, act_date, exp_date){
   a<-s5_msr_raw
   aa<-s5_productkey
   a2<-join(a, aa, by = 'Product Description')
   a2$pk2<-paste(a2$ProductDescriptor, a2$`Product Description`)
...
s5_filtered_p1_LAD_excluded<-s5_filtered_p1_LAD_InRange_func(s5_msr_raw = s5_msr_raw, act_date,
                                                             exp_date)
...
# 2) function output
ix_final<-ix_filtered_func(ix_mig_raw, act_date, exp_date)
...
write_csv(nf_final, 'nf_msr_2019-07-31_filtered-test.csv')
...
directi_raw <- read_csv(paste0(raw_msr_subs_directory, 'Directi_Monthly-Subs-Report_', filename_date, '.csv'))
...
write_csv(h_final, 'hg_brazil_msr_2019-07-31_filtered-test.csv')
...
write_csv(final, 'gator_msr_2019-07-31_filtered-test.csv')
...
write_csv(b.final, 'backup_msr_2019-07-31_filtered-test.csv')
...
write_csv(hs_final, 'homestead_msr_2019-07-31_filtered-test.csv')
...
write_csv(sp_final, 'sp_msr_2019-07-31_filtered-test.csv')
...
ctctsites_raw$Exp<-ctctsites_raw$`Term Expiration Date` + ctctsites_raw$NGS

h<-ctctsites_raw[!(ctctsites_raw$`Product Type` %in% c("ADTIME1", "ADTIME1.5", "ADTIME30", "AVEBOOK", "BCEBO", "BPEBOOK", 
                                                       "CVTEBO", "DESEBO", "DFSEBO", "IMAGE", "LOGO", "MANSSL", "OYUS", "PCLEBO",
                                                       "SEOEBO", "SEOPRO", "SSOEBO", "STPER", "STPRO", "STSMB", "UCEBO", "WTIEBO",
                                                       "DOMAIN", "FREEDOM", "FREEMAIL")),]
...
# CHANGE DATES !!!
h<-h[(h$`Activation Date` <= as.Date(act_date))
     & (h$`Activation Date` > as.Date(exp_date)) 
     & (h$`Last Authorization Date` <= as.Date(act_date))
     & (h$`Exp` >= (exp_date)),]
# CHECK DATE RANGES !!!
...
write_csv(final3, 'ctct-sites_msr_2019-05-31_filtered-test.csv')
